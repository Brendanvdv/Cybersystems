...
print("Welcome to the FSMD simulator! - Version 1 - Designed by Pablo, Abi and Brendan")
...
#This is a code snipped taken from the TAs.
...
#Initialize the state to false so it will not change state immediately. 
...
while cycle <= iterations:
    ...
    try:
        if (not(fsmd_stim['fsmdstimulus']['endstate'] is None)):
            if state == fsmd_stim['fsmdstimulus']['endstate']:
                print("The simulation has reached its end.")
                print(f"The number of cycles were {cycle}")
    ...
    # Allows the current state to transition to the next state.
    
    if Future_state:
        state = transition["nextstate"]
        Future_state = False 
        
    # iteration over the FSMD transition table.
    ...
    for transition in fsmd[state]:
        if evaluate_condition(transition["condition"]):
            ...
            print("The condition " + str(transition["condition"] + " is " + str(evaluate_condition(transition["condition"]))))
            print("Instruction: " + str(transition["instruction"]))
            print("Next state: " + str(transition["nextstate"]))
            print(f"At the end of cycle {cycle} the variables are {variables}")
            print("---------------------------\n")
            cycle += 1
        
            # Sets the future state to True so that the current state can move to the next state.  
    ...
    if iterations == cycle:
        print("The max number of iterations have been reached")
        print(f"The variables are {variables}")
        break
